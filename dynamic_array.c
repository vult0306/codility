...
{
    *(*(total_number_of_pages) + x*query + *(total_number_of_books+x))  = y;
...
int main()
{
    ...
    int *bookshell = (int*)malloc(total_number_of_shelves * total_number_of_queries * sizeof(int));    
    query = total_number_of_queries;
    total_number_of_pages = &bookshell;

    for (int i=0; i<total_number_of_shelves;i++)
        for (int j = 0; j < total_number_of_queries; j++)
            *(bookshell+i*total_number_of_shelves+j) = 1;
    for (int i=0; i<total_number_of_shelves;i++)
        for (int j = 0; j < total_number_of_queries; j++)        
            printf("%d ",*(bookshell+i*total_number_of_shelves + j));

    // for (int i=0; i<total_number_of_shelves;i++)
    //     for (int j = 0; j < total_number_of_queries; j++)
    //         *(*(total_number_of_pages+i*total_number_of_shelves)+j) = i+j;

    printf("\n");
    for (int i=0; i<total_number_of_shelves;i++)
        for (int j = 0; j < total_number_of_queries; j++)        
            printf("%d ",*(*total_number_of_pages + i*total_number_of_shelves + j));
    printf("\n");
